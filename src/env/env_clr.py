...
class RILAB_OMY_ENV:
    ...
    def reset(self, seed = None, leader_pose = True):
        ...
        if leader_pose:
            ...
            q_init = np.deg2rad([0.0] * len(self.joint_names))
            q_zero,ik_err_stack,ik_info = solve_ik(
                env = self.env,
                joint_names_for_ik = self.joint_names,
                body_name_trgt     = self.tcp_link_name,
        ...
        self.env.set_R_body(body_name='world', R=rpy2r(np.deg2rad(self.cfg['init_pose']['robot']['rotation'])))
    ...
    def render(self, task='', guideline='', fail_signal=False):
        self.env.plot_time()
        base_pos, base_R = self.env.get_pR_body(body_name='world')
    ...
    def get_object_pose(self, pad = None):
        ...
        recp_names = [name for name in recp_names]
