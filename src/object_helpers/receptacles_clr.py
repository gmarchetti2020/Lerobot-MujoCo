...
class Bench():
    def __init__(self, env):
        self.env = env
        self.obj_name = 'bench_lid'
        self.joint_name = "bench_lid_joint"
        self.state = False # self.door_state

    def check_range(self, condition="open"):
        joint = self.env.get_qpos_joint(self.joint_name)[0]
        joint_based_state = "close" if 1.65806 / 3.0  else "open"
        return condition == joint_based_state
    
    
        # if joint 
        # if condition == 'open' and joint < 1.65806 / 3.0: 
        #     return True
        # if condition == 'close' and joint > 0: 
        #     return True
        # else: 
        #     return False
    ...
    def reset(self, condition='close'):
        ...
        joint_idx = self.env.get_idxs_fwd(joint_names=[self.joint_name])[0]
        if condition == 'open':
            self.env.data.qpos[joint_idx] = 1.65806
    ...
    def get_q_state(self):
        joint_state = self.env.get_qpos_joint(self.joint_name)[0]
        return {self.joint_name: joint_state}
    
    def set_q_state(self, joint_states):
        joint_idx = self.env.get_idxs_fwd(joint_names=[self.joint_name])[0]
        self.env.data.qpos[joint_idx] = joint_states[self.joint_name]
